...
import { Droplet, LogIn, Mail, Lock, ShieldAlert, Chrome } from 'lucide-react';

const Login = () => {
  const { login, loginWithGoogle, error, setError, user } = useContext(AuthContext);
    ...
    <div className="max-w-md mx-auto px-6 py-16 relative">
      <div className="card-panel p-8 space-y-6" style={{ border: '1px solid var(--card-border)' }}>
        ...
        <div className="text-center text-xs" style={{ borderTop: '1px solid var(--card-border)', paddingTop: '12px', color: 'var(--text-muted)' }}>
          New to Jeevan 2.0?{' '}<Link to="/register" className="text-brand-600 hover:underline font-bold">Create Account</Link>
        </div>
      </div>
    </div>
  );
